...
3. Build an install URL with **Send Messages + Mention Everyone + Slash Commands** permissions (`permissions=133120`) and both `bot` + `applications.commands` scopes (the latter is required for slash commands; the Mention Everyone bit is required so `@everyone` reminders actually ping):
   ```
