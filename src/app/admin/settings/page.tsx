...
export default async function GuildSettingsPage({
  ...
  const [session, t, ta, te] = await Promise.all([
    auth(),
    getTranslations("guildSettingsPage"),
  ...
  if (!guild) return <p className="text-red-600 dark:text-red-300">{te("guildNotFound")}</p>;
    ...
    <div className="mx-auto max-w-2xl">
      {isImpersonating && (
        <div className="mb-4 rounded-md border border-amber-300 bg-amber-50 px-4 py-2 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-200">
          {ta("actingAsBanner", { guildName: guild.name })}
        </div>
      )}
      ...
      <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
        {t("slugPermanent", { slug: guild.slug })}
      </p>
