...
// Returns the earliest known start timestamp across simple/match modes, or
// null if nothing is scheduled. Used for "is the event upcoming?" sorting.
export function effectiveStartIso(event: EventLike): string | null {
  if (event.kind === "simple") return event.gameTime ?? null;
